...
1. 填写工作流名称、描述和保存路径。
2. 定义一个或多个输入文件。
3. 按顺序添加 `fill`、`delete`、`insert`、`merge`、`crc32` 或 `sha256` 处理步骤。
...
`delete` 将指定范围内已有字节改为 `0xFF`，不会移动后续偏移，也不会改变镜像长度。
...
范围采用半开区间 `[offset, offset + length)`。超出文件末尾的部分会被忽略，因此 `delete` 不会扩展或压缩 BIN 镜像。
