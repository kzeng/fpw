...
`.ffc` import is best-effort. MVP-supported steps are converted where possible. Unsupported FirmwareFlow steps produce warnings.

Current MVP importer behavior:
...
- `crc` -> `crc32`
- `sha256-bin` -> `sha256`
- `sha256` -> `sha256` + `insert`, with warning
- `insert` -> `insert`
- `merge` -> `merge`, with warning because FirmwareFlow range slicing is not preserved
- RSA/AES/HMAC-related steps are skipped with warnings
