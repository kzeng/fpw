...
import { emptyWorkflow, postbuildTemplate, type OpenWorkflow, type PostbuildTemplate, type Workflow, type WorkflowSummary } from "./workflow";
...
function App() {
  ...
  function beginTemplate(template: PostbuildTemplate) {
    const workflow = postbuildTemplate(template);
    setDraft(workflow);
    setDraftPath(workflowFileNameForTemplate(template));
    setIsNew(true);
    setView("wizard");
  }
  ...
  return (
    <main className="appShell">
      <header className="topBar">
        ...
        <div className="topUtilities">{serviceVersion ? <code className="versionBadge">v{serviceVersion}</code> : null}<div className="languageSwitch" aria-label="Language"><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "zh" ? "active" : ""} onClick={() => setLanguage("zh")}>中文</button></div><div className={`servicePill ${serviceStatus === "ok" ? "online" : ""}`}><Activity size={14} aria-hidden="true" /><span /> Core {serviceStatus}</div></div>
      </header>

      <div className="appContent">
        {view === "library" ? <LibraryView root={root} workflows={workflows} busy={busy} error={error} onNew={beginNew} onTemplate={beginTemplate} onEdit={(path) => load(path, "wizard")} onRun={(path) => load(path, "run")} onRefresh={refreshLibrary} onDuplicate={(source, target) => libraryAction(() => duplicateWorkflow(source, target))} onArchive={(path) => libraryAction(() => archiveWorkflow(path))} onImport={(kind, source, target) => libraryAction(() => importWorkflow(kind, source, target))} /> : null}
...
function workflowFileNameForTemplate(template: PostbuildTemplate): string {
  if (template === "mcu") return "postbuild-mcu.fwp";
  if (template === "dsp") return "postbuild-dsp.fwp";
  return "postbuild-nvr.fwp";
}
