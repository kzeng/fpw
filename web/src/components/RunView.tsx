...
import { ArrowLeft, ClipboardList, Eye, FileInput, FileOutput, LoaderCircle, Pencil, Play, ShieldCheck } from "lucide-react";
...
export function RunView({ selected, onBack, onEdit }: Props) {
  ...
  return (
    <section className="runView">
      <header className="viewHeader compactHeader"><div><span className="eyebrow">Controlled execution</span><h2>{t("Run {name}", { name: selected.workflow.name })}</h2><p>{selected.workflow.description || selected.path}</p></div><div className="headerActions"><button onClick={onBack}><ArrowLeft size={16} aria-hidden="true" />{t("Back to library")}</button><button onClick={onEdit}><Pencil size={16} aria-hidden="true" />{t("Edit configuration")}</button></div></header>
      <div className="runLayout">
        ...
        <section className="runConfiguration">
          <div className="runSection"><span className="sectionLabel">01 · {t("Inputs")}</span><h3 className="sectionHeading"><FileInput size={19} aria-hidden="true" />{t("Choose inputs for this run")}</h3>{inputs.map((step) => <label key={step.id}>{String(step.name)}<input value={inputPaths[String(step.name)] ?? ""} onChange={(event) => setInputPaths((current) => ({ ...current, [String(step.name)]: event.target.value }))} placeholder={t("Default: {path}", { path: String(step.path ?? t("Not configured")) })} /><small>{t("Leave blank to use the default path from the .fwp file.")}</small></label>)}</div>
          <div className="runSection"><span className="sectionLabel">02 · {t("Outputs")}</span><h3 className="sectionHeading"><FileOutput size={19} aria-hidden="true" />{t("Confirm output locations")}</h3>{outputs.map((step) => <label key={step.id}>{String(step.name)}<input value={outputPaths[String(step.name)] ?? ""} onChange={(event) => setOutputPaths((current) => ({ ...current, [String(step.name)]: event.target.value }))} placeholder={t("Default: {path}", { path: String(step.path ?? t("Not configured")) })} /><small>{t("Leave blank to resolve relative to the .fwp directory.")}</small></label>)}</div>
          <div className="runSection"><span className="sectionLabel">03 · {t("Reports")}</span><h3 className="sectionHeading"><ClipboardList size={19} aria-hidden="true" />{t("Execution records")}</h3><label>{t("Report directory")}<input value={reportDir} onChange={(event) => setReportDir(event.target.value)} /></label></div>
          <div className="runCommandBar"><button disabled={Boolean(busy)} onClick={() => execute("validate")}>{busy === "validate" ? <LoaderCircle className="isSpinning" size={16} aria-hidden="true" /> : <ShieldCheck size={16} aria-hidden="true" />}{t(busy === "validate" ? "Validating..." : "Validate")}</button><button disabled={Boolean(busy)} onClick={() => execute("preview")}>{busy === "preview" ? <LoaderCircle className="isSpinning" size={16} aria-hidden="true" /> : <Eye size={16} aria-hidden="true" />}{t(busy === "preview" ? "Previewing..." : "Preview")}</button><button className="primaryButton" disabled={Boolean(busy)} onClick={() => execute("run")}>{busy === "run" ? <LoaderCircle className="isSpinning" size={16} aria-hidden="true" /> : <Play size={16} aria-hidden="true" />}{t(busy === "run" ? "Running..." : "Run workflow")}</button></div>
        </section>
      </div>
